...
export function ProjectJourney({ current = "discover" }: { current?: JourneyStage }) {
  return (
    <section className="section-shell relative z-10 -mt-5 sm:-mt-7">
      <div className="rounded-[1.6rem] border border-white bg-white/95 p-3 shadow-[0_24px_80px_rgba(15,23,42,0.13)] backdrop-blur-xl">
        <div className="mb-3 flex items-center justify-between gap-3 px-2 pt-1">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500">Your simple path</p>
        ...
        <div className="grid gap-2 md:grid-cols-4">
          {stages.map(({ id, label, copy, href, icon: Icon }, index) => {
            ...
            return (
              <Link key={id} href={href} className={`group flex min-w-0 items-center gap-3 rounded-2xl border p-3 transition duration-300 hover:-translate-y-0.5 ${active ? "border-sky-200 bg-sky-50 shadow-sm" : "border-transparent bg-slate-50/80 hover:border-slate-200 hover:bg-white"}`}>
                <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${active ? "bg-sky-600 text-white" : "bg-white text-slate-500 shadow-sm group-hover:text-sky-600"}`}>
                  {active ? <CheckCircle2 className="h-5 w-5" /> : <Icon className="h-5 w-5" />}
                </span>
                <span className="min-w-0">
                  <span className="block text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Step {index + 1}</span>

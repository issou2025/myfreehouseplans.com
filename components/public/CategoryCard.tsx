...
export function CategoryCard({ category }: { category: Category }) {
  return (
    <Link href={`/plans?category=${category.slug}`} className="block h-full">
      <Card className="group relative h-full min-h-64 overflow-hidden border-0 bg-slate-950 p-0 transition hover:-translate-y-1 hover:shadow-[0_30px_80px_rgba(15,23,42,0.22)]">
        ...
        <div className="relative flex h-full min-h-64 flex-col justify-between p-5">
          <div className="flex items-start justify-between gap-3">
            <span className="grid h-11 w-11 place-items-center rounded-2xl border border-white/20 bg-white/15 text-white backdrop-blur"><Home className="h-5 w-5" /></span>
            <span className="rounded-full border border-white/20 bg-white/15 p-2 text-white backdrop-blur transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5"><ArrowUpRight className="h-4 w-4" /></span>
          </div>
          <div>
            <p className="text-lg font-black text-white sm:text-xl">{category.name}</p>

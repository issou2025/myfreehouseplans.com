  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="mt-12"><h2 className="text-3xl font-black text-slate-950">FAQ</h2><div className="mt-5 grid gap-3">{["What professional review is required?", "What is included in the free PDF?", "What is the difference between PDF, DWG, Revit and IFC?", "Can this concept be adapted?", `How should I evaluate this concept for a ${formatPlotDual(plan.plotWidth, plan.plotLength, true)} plot?`].map((q) => <Card key={q} className="p-5"><p className="font-bold text-slate-950">{q}</p><p className="mt-2 text-sm leading-6 text-slate-600">This page and its files are project information for review. A qualified local professional must verify and adapt them to the site, regulations, structure and project requirements.</p></Card>)}</div></section>
        <NextStepCTA />
        <RelatedTools />
        <RelatedArticles />
